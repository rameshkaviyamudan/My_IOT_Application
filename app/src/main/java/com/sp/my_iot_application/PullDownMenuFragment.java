...
public class PullDownMenuFragment extends Fragment {
    ...
    private void populateDeviceSpinner() {
        ...
        List<String> devices = Arrays.asList("Fan", "Lamp");
    ...
    private void scheduleNotification() {
        try {
            ...
            // Determine the field numbers and values based on the selected device and status
            int fieldNumber = -1;
            int fieldValue = -1;

            if ("Fan".equals(selectedDevice)) {
                fieldNumber = 4;
            } else if ("Lamp".equals(selectedDevice)) {
                fieldNumber = 5;
            }

            // Convert the status to lowercase for consistency
            String lowerCaseStatus = selectedStatus.toLowerCase();

            // Set the field value based on the lowercase status
            fieldValue = "on".equals(lowerCaseStatus) ? 1 : 0;

            // Make the API call with the determined field number and value
            if (fieldNumber != -1) {
                makeApiCall(fieldNumber, fieldValue, hour, minute);


                // Store the selected time, device, and status values
...
// Store the values using SharedPreferences
                ...
                editor.putInt("fieldNumber", fieldNumber);
                editor.putInt("fieldValue", fieldValue);
                editor.apply();
